...
fn eqv() {
    ...
    assert_eq!(
        ...
        eval(
            &Expression::from_vec(vec![
                Expression::Symbol(Symbol::new("eqv?")),
                Expression::Char('x'),
                Expression::Char('x')
            ]),
            default_env()
    ...
    assert_eq!(
        Expression::True,
        run("(define x (cons 1 2)) (eqv? x x)").unwrap()
    );
    assert_eq!(
        Expression::True,
        run("(define x \"xyz\") (eqv? x x)").unwrap()
    );
    assert_eq!(Expression::True, run("(eqv? + +)").unwrap());
    assert_eq!(
        Expression::True,
    ...
    assert_eq!(
        Expression::False,
        run("(eqv? (cons 1 2) (cons 1 2))").unwrap()
    );
...
fn equal() {
    ...
    assert_eq!(
        Expression::True,
        run("(equal? (lambda (x) (* x x)) (lambda (x) (* x x)))").unwrap()
    );
