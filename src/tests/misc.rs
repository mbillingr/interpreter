...
fn type_predicates() {
    ...
    assert_eq!(
        run_in_env("(null? '())", &env).unwrap(),
        Expression::True,
    );
    
    assert_eq!(
        run_in_env("(null? 0)", &env).unwrap(),
        Expression::False,
    );
    
    assert_eq!(
        run_in_env("(boolean? 1)", &env).unwrap(),
        Expression::False,
    );
    
    assert_eq!(
        run_in_env("(boolean? #f)", &env).unwrap(),
        Expression::True,
    );
    
    assert_eq!(
        run_in_env("(boolean? #t)", &env).unwrap(),
        Expression::True,
    );
    ...
    assert_eq!(
        run_in_env("(complex? 12)", &env).unwrap(),
        Expression::True,
    );
    ...
    assert_eq!(
        run_in_env("(number? 1/2)", &env).unwrap(),
        Expression::True,
    );
    ...
    assert_eq!(
        run_in_env("(real? 1/2)", &env).unwrap(),
        Expression::True,
    );
    ...
    assert_eq!(
        run_in_env("(real? 1+2i)", &env).unwrap(),
        Expression::False,
    );
