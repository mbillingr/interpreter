...
pub fn run<F: FnMut(&Ui) -> bool>(title: String, clear_color: [f32; 4], mut run_ui: F) {
    ...
    let (context, mut device, mut factory, mut main_color, mut main_depth) =
    ...
    let hidpi_factor = context.window().get_hidpi_factor().round();
    ...
    loop {
        events_loop.poll_events(|event| {
            ...
            if let Event::WindowEvent { event, .. } = event {
                match event {
                    Resized(_) => {
                        gfx_window_glutin::update_views(&context, &mut main_color, &mut main_depth);
