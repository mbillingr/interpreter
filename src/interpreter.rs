...
pub fn inner_eval(expr: &Expression, mut env: EnvRef) -> Result<Expression> {
    ...
    loop {
        ...
        let retval = match *expr {
            ...
            Undefined | Nil | Number(_) | String(_) | Char(_) | True | False | Procedure(_)
            | Macro(_) | Special(_) | Vector(_) | OpaqueVector(_) | File(_) | Class(_)
            | Instance(_) => Return::Value(expr.into_owned()),
